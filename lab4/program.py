...
def query_evaluation(queries, bayes_network):
    ...
    for query in queries:
        ...
        parents_numerator = []
        ancestors_numerator = bayes_network.get_ancestors(
            list_conditions,  [], parents_numerator)

        combinations_numerator = get_all_combinations(ancestors_numerator)
        numerator = 0
        ...
        if len(query[1]) > 0:
            ...
            for q in query[1]:
                list_conditions.append(q[1:])
            parents_denominator = []
            ancestors_denominator = bayes_network.get_ancestors(
                list_conditions, [], parents_denominator)
            combinations_denominator = get_all_combinations(
                ancestors_denominator)
            denominator = 0
...
def get_combinations(probabilities, start, end, combinations, sign, index):
    ''' form combinations of probabilities '''
...
def get_all_combinations(probabilities):
    ...
    get_combinations(probabilities, 0, int(
        num_combinations/2), combinations, '+', 0)
    get_combinations(probabilities, int(num_combinations/2),
                     num_combinations, combinations, '-', 0)
    return combinations
