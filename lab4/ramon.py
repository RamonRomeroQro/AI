...
class Network():
	...
	def get_more_nodes(self, nodes_with_sign):
		node_names= set([ x[1:] if x.startswith('+') or x.startswith('-')  else x for x in nodes_with_sign ])
		extra=set()
		for node_name in node_names:
			node = self.get_node_by_name(node_name)
			if len(node.parents)!=0:
				for p in node.parents.keys():
					if p in node_names:
						extra.add(p)
			else:
				pass
		return node_names|extra
...
def getWithHiddenNodes(nodes, bayes_network):
    result = copy.deepcopy(nodes)
    for i in range(len(result)):
        result[i] = result[i].replace('+','').replace('-', '')

    for name in result:
        node = bayes_network.get_node_by_name(name)
        if len(node.parents)!=0:
...
def compute_probability(query, bayes_network):
	...
	unsigned = []
	if(len(query_array) > 1 ):
		total_related_nodes = getWithHiddenNodes(query_array, bayes_network)
		unsigned_query_array = [ x[1:] if x.startswith('+') or x.startswith('-')  else x for x in query_array ]
		hidden = list(( set(total_related_nodes) -  set(unsigned_query_array) | ( set(unsigned_query_array)- set(total_related_nodes))))
		all_with_signs = []
		signs =  ['+', '-']
		...
		if(len(product_hidden) >0 ):
			x = [ str(e[0])+str(e[1]) for e in product_hidden]
			product = []
			newqueries = []
