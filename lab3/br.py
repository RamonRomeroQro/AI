...
def a_star(start_node, goal_node, gsp):
    ...
    pq = PriorityQueue()
    # nodo, currentsume, path, heuristica
    pq.add((gsp.nodes[start_node.id], 0, [], gsp.heuristic[start_node.id]))
    kek=0
    while(True):
        kek=kek+1
        print(str(kek))
        ...
        for t, n in nodo.conections.items():  # expansion
            ...
            if conex.id not in explored:  # or conex not frontier:
                # print(conex)
                explored[conex.id] = nodo
                sortk=(cost_n+latest_cost)+gsp.heuristic[start_node.id]
...
def main():
    ...
    if greedy == True:
        #print(ucs_greedy(initialState, goalState, gsp))
        ...
        if goalState.id in gsp.nodes:
            #print("Found")
            #print(ucs(initialState, goalState,gsp))
            gsp.generate_heuristic(goalState.structure)
